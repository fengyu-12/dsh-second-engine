...
`
    ...
    function ProviderCard() {
      ...
      try {
        rows = providers.map((p) => {
          ...
          const isPreset = p.id === 'deepseek' || p.id === 'zhipu' || p.id === 'yun'
    ...
    function ReviewSection(props) {
      ...
      // 面板打开时拉列表（只看复核）；有 running 时每 4 秒续拉。
      React.useEffect(() => {
      ...
      const submit = async () => {
        ...
        if (text === '') { setNotice({ kind: 'err', text: '被审产出不能为空' }); return }
        setBusy(true); setNotice(null)
        try {
          const r = await postJson('/second-engine/api/review', { content: text, rounds: Number(rounds) })
          if (r && r.ok) {
            setContent('')
            setNotice({ kind: 'ok', text: `已提交复核 ${r.id} · 最多 ${r.maxRounds} 轮` })
            setVersion((v) => v + 1)
          } else {
            setNotice({ kind: 'err', text: (r && r.error) || '提交复核失败' })
          }
        } catch (e) { setNotice({ kind: 'err', text: `提交复核失败：${errText(e)}` }) }
        setBusy(false)
      }
      ...
      return h('div', { className: 'dse-ball-tasks' },
        h('textarea', {
          ...
          rows: 4,
          placeholder: '被审产出文本（服务层将循环调用独立复核引擎批判性复核）',
          value: content,
          onChange: (e) => { try { setContent(e.target.value) } catch { /* best effort */ } },
        }),
        h('div', { className: 'dse-ball-row' },
          h('select', {
            ...
            value: String(rounds),
            'aria-label': '复核轮数',
            onChange: (e) => { try { setRounds(Number(e.target.value) === 5 ? 5 : 3) } catch { /* best effort */ } },
          },
    ...
    function TaskSection(props) {
      ...
      const submit = async () => {
        ...
        setBusy(true); setNotice(null)
        try {
          const r = await postJson('/second-engine/api/task', { prompt: text })
          if (r && r.ok) {
            setPrompt('')
