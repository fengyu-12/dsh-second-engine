    ...
    function StatusCard() {
      ...
      React.useEffect(() => {
        ...
        getJson('/second-engine/api/version').then((r) => {
          if (alive && r && r.ok) setVer({ local: r.local, latest: r.latest, updateAvailable: !!r.updateAvailable, htmlUrl: r.htmlUrl || 'https://github.com/openai/codex/releases/latest' })
