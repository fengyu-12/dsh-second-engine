...
.dse-card-head-click{cursor:pointer}
.dse-card-pill{flex:none;padding:3px 12px;border:1px solid var(--dsw-alias-border-l2);border-radius:999px;background:var(--dsw-alias-bg-layer-2);color:var(--dsw-alias-label-secondary);font-size:11px;line-height:1.4;cursor:pointer;user-select:none;-webkit-user-select:none}
...
.dse-ball-tasklist{display:flex;flex-direction:column;gap:4px;margin-top:2px;max-height:38vh;overflow-y:auto;overscroll-behavior:contain;-webkit-overflow-scrolling:touch}
.dse-ball-history-toggle{align-self:flex-start;font:inherit;font-size:11px;line-height:1.6;color:var(--dsw-alias-label-secondary);background:transparent;border:1px solid var(--dsw-alias-border-l2);border-radius:999px;padding:2px 10px;cursor:pointer;user-select:none;-webkit-user-select:none}
...
`
    ...
    function Card(props) {
      ...
      return h('div', { className: 'dse-card' },
        h('div', {
          className: 'dse-card-head' + (collapsible ? ' dse-card-head-click' : ''),
    ...
    function TaskSection(props) {
      ...
      const [busy, setBusy] = React.useState(false)
      const [version, setVersion] = React.useState(0)
      const [showAllTasks, setShowAllTasks] = React.useState(false) // 历史/运行中分栏：默认只显示运行中+最近4条已完成
      ...
      let rows = null
      let historyBtn = null // try 外的渲染处要引用，须在外层声明（教训：try 块内 const 是块级作用域）
      try {
        // 列表瘦身：默认只显示运行中 + 最近 4 条已完成，其余折叠（防列表无限拉长把面板头部挤出视野）
        const runningTasks = list.filter((t) => t && t.status === 'running')
        const doneTasks = list.filter((t) => !t || t.status !== 'running')
        let visible = showAllTasks ? list : [...runningTasks, ...doneTasks.slice(0, 4)]
        if (expanded !== '') {
          const et = list.find((t) => t && t.id === expanded)
          if (et && !visible.some((t) => t && t.id === expanded)) visible = [...visible, et]
        }
        const hiddenCount = list.length - visible.length
        historyBtn = hiddenCount > 0 && !showAllTasks
          ? h('button', {
              type: 'button',
              className: 'dse-ball-history-toggle',
              onClick: () => { try { setShowAllTasks(true) } catch { /* best effort */ } },
            }, `历史工单（${hiddenCount}）·展开`)
          : (showAllTasks && doneTasks.length > 4
              ? h('button', {
                  type: 'button',
                  className: 'dse-ball-history-toggle',
                  onClick: () => { try { setShowAllTasks(false) } catch { /* best effort */ } },
                }, '收起历史工单')
              : null)
        rows = visible.map((task, i) => {
          const t = task && typeof task === 'object' ? task : {}
          const id = typeof t.id === 'string' ? t.id : ''
      ...
      return h('div', { className: 'dse-ball-tasks' },
        ...
        rows !== null && rows.length > 0
          ? h('div', { className: 'dse-ball-tasklist' }, rows, historyBtn)
